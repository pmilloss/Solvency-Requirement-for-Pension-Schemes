...
for (trun in 1:totalruns) {
  ...
  #Here we first fit RH model to EW date. The code is from StMoMo vignette
  ...
  RHfit <- fit(RH, data = EWMaleIniData,  ages.fit = ages.fit, wxt = wxt,  start.ax = LCfit$ax, start.bx = LCfit$bx, start.kt = LCfit$kt)
  
  RHres <- residuals(RHfit)
  
  RHfor <- forecast(RHfit, h = 37, gc.order = c(1, 1, 0))
  
  #Here I create the column of mortality rates (q_x) from the central forecasts for the cohort that is 65 in 2012. This is used to caRHulate premium and reserves
  q_xRH <- extractCohort(A = RHfor$rates, age = 65, period = 2012)
  
  ######## Here I linearly interpolated the  mortality rates from age 100 to age 120 (when q_x=1)
  
  #coefficient<-(1-q_xRH1[36,1])/20
  
  # quadratic interpolation
  
  
  quadrinterp <- poly.calc(x = c(99, 100, 120), y = c(q_xRH[35], q_xRH[36], 1))
  q_xRH[36 + (1 : 20)] <- predict(quadrinterp, 101 : 120)
  #points(q_xRH, col = "red")
  
  
  #q_xRH3=c()
  #for (i in 2:20) {
  # q_xRH3[1]<-q_xRH1[36,1]+coefficient
  # q_xRH3[i]<-q_xRH3[(i-1)]+coefficient
  
  #}
  
  #q_xRH2<-as.data.frame(q_xRH3)
  #colnames(q_xRH2)<-"CentralRH"
  #q_xRH<-rbind(q_xRH1,q_xRH2)
  ...
  # Names is differen here. "Central RH"
  names(q_xRH) <- names(p_xRH) <- seq(65, 120)
  
  
  
  tt <- seq(0, length(p_xRH))
  
  # cumulative survival rates (USE q_xRH AS A VECTOR RATHER THAN A DATA FRAME)
  tp_xRH <- cumprod(c(1, p_xRH))
  
  # cashflows at times 0, 1, 2, ... (I DON'T REMEMBER IF IT WAS IMMEDIATE OR DUE :) THIS ONE BELOW IS IMMEDIATE)
  annuity_cashflows <- c(rep(0, deferral + 1), rep(R, length.out = length(p_xRH) - deferral))
  # annuity_cashflows <- c(rep(0, deferral), rep(1, length.out = length(p_xRH) - deferral + 1)) # THIS IF YOU WANT ANNUITY DUE
  
  
  # discount factors
  B0t <- (1 + InterestRate) ^ - tt
  
  names(tp_xRH) <- names(annuity_cashflows) <- names(B0t) <- tt
  
  premium <- n*sum(annuity_cashflows * tp_xRH * B0t)
  
  
  
  ############################
  # CaRHulation backward #####
  ############################
  ...
  for (i in tail(tt, 1) : 1)
  {
    ...
    # caRHulate V(t-1) from V(t)
    V[ti0] <- ((V[ti1] + (annuity_cashflows[ti1])/R) * p_xRH[xt]) / (1 + InterestRate) - Pt[ti0]
  }
  ...
  RHsim <- simulate(RHfit, nsim = runs, h = 37, gc.order = c(1, 1, 0))
  ...
  RHsim2012<- extractCohort(RHsim$rates, age = EntryAge, period = 2012)
  forinterpolation<-as.matrix(rbind(RHsim2012[35,], RHsim2012[36,], rep(1,runs)))
  ...
  RHcohort2012<-rbind(RHsim2012, forsimulations)
  RHcohort2012[RHcohort2012 > 1] <-1 
  rownames(RHcohort2012)<-65:120
  ...
  survivals[1,]<-n
  for (i in 1:56) {
    survivals[(i+1),]<-rbinom(runs,survivals[i,],1-RHcohort2012[i,])
  ...
  lr1st<-rep(0,runs)
  #CaRHulate random payments to policyholders considering random number of alives
  ...
  #CaRHulate random portfolio fund at times t, i.e. Z_t
  ...
  #CaRHulate solvency margin that needs to be added in the beginning in order to ensure solvence at time t. This is basically a quantile of dicounted deficit
  ...
  # cashflows at times 0, 1, 2, ... (I DON'T REMEMBER IF IT WAS IMMEDIATE OR DUE :) THIS ONE BELOW IS IMMEDIATE)
  annuity_cashflows2 <- c(rep(0, deferral2 + 1), rep(R, length.out = length(p_xRH) - deferral2))
  # annuity_cashflows <- c(rep(0, deferral2), rep(1, length.out = length(p_xRH) - deferral2 + 1)) # THIS IF YOU WANT ANNUITY DUE
  
  
  names(tp_xRH) <- names(annuity_cashflows2) <- names(B0t) <- tt
  
  
  
  
  premium2 <- n*sum(annuity_cashflows2 * tp_xRH * B0t)
  ...
  for (i in tail(tt, 1) : 1)
  {
    ...
    # caRHulate V(t-1) from V(t)
    V2[ti0] <- ((V2[ti1] + (annuity_cashflows2[ti1])/R) * p_xRH[xt]) / (1 + InterestRate) - Pt2[ti0]
  }
  ...
  # cashflows at times 0, 1, 2, ... (I DON'T REMEMBER IF IT WAS IMMEDIATE OR DUE :) THIS ONE BELOW IS IMMEDIATE)
  annuity_cashflows3 <- c(rep(0, deferral3 + 1), rep(R, length.out = length(p_xRH) - deferral3))
  # annuity_cashflows <- c(rep(0, deferral2), rep(1, length.out = length(p_xRH) - deferral2 + 1)) # THIS IF YOU WANT ANNUITY DUE
  
  
  names(tp_xRH) <- names(annuity_cashflows3) <- names(B0t) <- tt
  
  
  
  
  premium3 <- n*sum(annuity_cashflows3 * tp_xRH * B0t)
  ...
  for (i in tail(tt, 1) : 1)
  {
    ...
    # caRHulate V(t-1) from V(t)
    V3[ti0] <- ((V3[ti1] + (annuity_cashflows3[ti1])/R) * p_xRH[xt]) / (1 + InterestRate) - Pt3[ti0]
  }
  ...
  # cashflows at times 0, 1, 2, ... (I DON'T REMEMBER IF IT WAS IMMEDIATE OR DUE :) THIS ONE BELOW IS IMMEDIATE)
  annuity_cashflows4 <- c(rep(0, deferral4 + 1), rep(R, length.out = length(p_xRH) - deferral4))
  # annuity_cashflows <- c(rep(0, deferral2), rep(1, length.out = length(p_xRH) - deferral2 + 1)) # THIS IF YOU WANT ANNUITY DUE
  
  
  names(tp_xRH) <- names(annuity_cashflows4) <- names(B0t) <- tt
  
  
  
  
  premium4 <- n*sum(annuity_cashflows4 * tp_xRH * B0t)
  ...
  for (i in tail(tt, 1) : 1)
  {
    ...
    # caRHulate V(t-1) from V(t)
    V4[ti0] <- ((V4[ti1] + (annuity_cashflows4[ti1])/R) * p_xRH[xt]) / (1 + InterestRate) - Pt4[ti0]
  }
  ...
  # cashflows at times 0, 1, 2, ... (I DON'T REMEMBER IF IT WAS IMMEDIATE OR DUE :) THIS ONE BELOW IS IMMEDIATE)
  annuity_cashflows5 <- c(rep(0, deferral5 + 1), rep(R, length.out = length(p_xRH) - deferral5))
  # annuity_cashflows <- c(rep(0, deferral2), rep(1, length.out = length(p_xRH) - deferral2 + 1)) # THIS IF YOU WANT ANNUITY DUE
  
  
  names(tp_xRH) <- names(annuity_cashflows5) <- names(B0t) <- tt
  
  
  
  
  premium5 <- n*sum(annuity_cashflows5 * tp_xRH * B0t)
  ...
  for (i in tail(tt, 1) : 1)
  {
    ...
    # caRHulate V(t-1) from V(t)
    V5[ti0] <- ((V5[ti1] + (annuity_cashflows5[ti1])/R) * p_xRH[xt]) / (1 + InterestRate) - Pt5[ti0]
  }
  ...
  # cashflows at times 0, 1, 2, ... (I DON'T REMEMBER IF IT WAS IMMEDIATE OR DUE :) THIS ONE BELOW IS IMMEDIATE)
  annuity_cashflows6 <- c(rep(0, deferral6 + 1), rep(R, length.out = length(p_xRH) - deferral6))
  # annuity_cashflows <- c(rep(0, deferral2), rep(1, length.out = length(p_xRH) - deferral2 + 1)) # THIS IF YOU WANT ANNUITY DUE
  
  
  names(tp_xRH) <- names(annuity_cashflows6) <- names(B0t) <- tt
  
  
  
  
  premium6 <- n*sum(annuity_cashflows6 * tp_xRH * B0t)
  ...
  for (i in tail(tt, 1) : 1)
  {
    ...
    # caRHulate V(t-1) from V(t)
    V6[ti0] <- ((V6[ti1] + (annuity_cashflows6[ti1])/R) * p_xRH[xt]) / (1 + InterestRate) - Pt6[ti0]
  }
  ...
  # cashflows at times 0, 1, 2, ... (I DON'T REMEMBER IF IT WAS IMMEDIATE OR DUE :) THIS ONE BELOW IS IMMEDIATE)
  annuity_cashflows7 <- c(rep(0, deferral7 + 1), rep(R, length.out = length(p_xRH) - deferral7))
  # annuity_cashflows <- c(rep(0, deferral2), rep(1, length.out = length(p_xRH) - deferral2 + 1)) # THIS IF YOU WANT ANNUITY DUE
  
  
  names(tp_xRH) <- names(annuity_cashflows7) <- names(B0t) <- tt
  
  
  
  
  premium7 <- n*sum(annuity_cashflows7 * tp_xRH * B0t)
  ...
  for (i in tail(tt, 1) : 1)
  {
    ...
    # caRHulate V(t-1) from V(t)
    V7[ti0] <- ((V7[ti1] + (annuity_cashflows7[ti1])/R) * p_xRH[xt]) / (1 + InterestRate) - Pt7[ti0]
  }
  ...
  # cashflows at times 0, 1, 2, ... (I DON'T REMEMBER IF IT WAS IMMEDIATE OR DUE :) THIS ONE BELOW IS IMMEDIATE)
  annuity_cashflows8 <- c(rep(0, deferral8 + 1), rep(R, length.out = length(p_xRH) - deferral8))
  # annuity_cashflows <- c(rep(0, deferral2), rep(1, length.out = length(p_xRH) - deferral2 + 1)) # THIS IF YOU WANT ANNUITY DUE
  
  
  names(tp_xRH) <- names(annuity_cashflows8) <- names(B0t) <- tt
  
  
  
  
  premium8 <- n*sum(annuity_cashflows8 * tp_xRH * B0t)
  ...
  for (i in tail(tt, 1) : 1)
  {
    ...
    # caRHulate V(t-1) from V(t)
    V8[ti0] <- ((V8[ti1] + (annuity_cashflows8[ti1])/R) * p_xRH[xt]) / (1 + InterestRate) - Pt8[ti0]
  }
  ...
  # cashflows at times 0, 1, 2, ... (I DON'T REMEMBER IF IT WAS IMMEDIATE OR DUE :) THIS ONE BELOW IS IMMEDIATE)
  annuity_cashflows9 <- c(rep(0, deferral9 + 1), rep(R, length.out = length(p_xRH) - deferral9))
  # annuity_cashflows <- c(rep(0, deferral2), rep(1, length.out = length(p_xRH) - deferral2 + 1)) # THIS IF YOU WANT ANNUITY DUE
  
  
  names(tp_xRH) <- names(annuity_cashflows9) <- names(B0t) <- tt
  
  
  
  
  premium9 <- n*sum(annuity_cashflows9 * tp_xRH * B0t)
  ...
  for (i in tail(tt, 1) : 1)
  {
    ...
    # caRHulate V(t-1) from V(t)
    V9[ti0] <- ((V9[ti1] + (annuity_cashflows9[ti1])/R) * p_xRH[xt]) / (1 + InterestRate) - Pt9[ti0]
  }
  ...
  # cashflows at times 0, 1, 2, ... (I DON'T REMEMBER IF IT WAS IMMEDIATE OR DUE :) THIS ONE BELOW IS IMMEDIATE)
  annuity_cashflows10 <- c(rep(0, deferral10 + 1), rep(R, length.out = length(p_xRH) - deferral10))
  # annuity_cashflows <- c(rep(0, deferral2), rep(1, length.out = length(p_xRH) - deferral2 + 1)) # THIS IF YOU WANT ANNUITY DUE
  
  
  names(tp_xRH) <- names(annuity_cashflows10) <- names(B0t) <- tt
  
  
  
  
  premium10 <- n*sum(annuity_cashflows10 * tp_xRH * B0t)
  ...
  for (i in tail(tt, 1) : 1)
  {
    ...
    # caRHulate V(t-1) from V(t)
    V10[ti0] <- ((V10[ti1] + (annuity_cashflows10[ti1])/R) * p_xRH[xt]) / (1 + InterestRate) - Pt10[ti0]
  }
  ...
  # cashflows at times 0, 1, 2, ... (I DON'T REMEMBER IF IT WAS IMMEDIATE OR DUE :) THIS ONE BELOW IS IMMEDIATE)
  annuity_cashflows11 <- c(rep(0, deferral11 + 1), rep(R, length.out = length(p_xRH) - deferral11))
  # annuity_cashflows <- c(rep(0, deferral2), rep(1, length.out = length(p_xRH) - deferral2 + 1)) # THIS IF YOU WANT ANNUITY DUE
  
  
  names(tp_xRH) <- names(annuity_cashflows11) <- names(B0t) <- tt
  
  
  
  
  premium11 <- n*sum(annuity_cashflows11 * tp_xRH * B0t)
  ...
  for (i in tail(tt, 1) : 1)
  {
    ...
    # caRHulate V(t-1) from V(t)
    V11[ti0] <- ((V11[ti1] + (annuity_cashflows11[ti1])/R) * p_xRH[xt]) / (1 + InterestRate) - Pt11[ti0]
  }
  ...
  # cashflows at times 0, 1, 2, ... (I DON'T REMEMBER IF IT WAS IMMEDIATE OR DUE :) THIS ONE BELOW IS IMMEDIATE)
  annuity_cashflows14 <- c(rep(0, deferral14 + 1), rep(R, length.out = length(p_xRH) - deferral14))
  # annuity_cashflows <- c(rep(0, deferral2), rep(1, length.out = length(p_xRH) - deferral2 + 1)) # THIS IF YOU WANT ANNUITY DUE
  
  
  names(tp_xRH) <- names(annuity_cashflows14) <- names(B0t) <- tt
  
  
  
  
  premium14 <- n*sum(annuity_cashflows14 * tp_xRH * B0t)
  ...
  for (i in tail(tt, 1) : 1)
  {
    ...
    # caRHulate V(t-1) from V(t)
    V14[ti0] <- ((V14[ti1] + (annuity_cashflows14[ti1])/R) * p_xRH[xt]) / (1 + InterestRate) - Pt14[ti0]
  }
  ...
  # cashflows at times 0, 1, 2, ... (I DON'T REMEMBER IF IT WAS IMMEDIATE OR DUE :) THIS ONE BELOW IS IMMEDIATE)
  annuity_cashflows12 <- c(rep(0, deferral12 + 1), rep(R, length.out = length(p_xRH) - deferral12))
  # annuity_cashflows <- c(rep(0, deferral2), rep(1, length.out = length(p_xRH) - deferral2 + 1)) # THIS IF YOU WANT ANNUITY DUE
  
  
  names(tp_xRH) <- names(annuity_cashflows12) <- names(B0t) <- tt
  
  
  
  
  premium12 <- n*sum(annuity_cashflows12 * tp_xRH * B0t)
  ...
  for (i in tail(tt, 1) : 1)
  {
    ...
    # caRHulate V(t-1) from V(t)
    V12[ti0] <- ((V12[ti1] + (annuity_cashflows12[ti1])/R) * p_xRH[xt]) / (1 + InterestRate) - Pt12[ti0]
  }
  ...
  # cashflows at times 0, 1, 2, ... (I DON'T REMEMBER IF IT WAS IMMEDIATE OR DUE :) THIS ONE BELOW IS IMMEDIATE)
  annuity_cashflows15 <- c(rep(0, deferral15 + 1), rep(R, length.out = length(p_xRH) - deferral15))
  # annuity_cashflows <- c(rep(0, deferral2), rep(1, length.out = length(p_xRH) - deferral2 + 1)) # THIS IF YOU WANT ANNUITY DUE
  
  
  names(tp_xRH) <- names(annuity_cashflows15) <- names(B0t) <- tt
  
  
  
  
  premium15 <- n*sum(annuity_cashflows15 * tp_xRH * B0t)
  ...
  for (i in tail(tt, 1) : 1)
  {
    ...
    # caRHulate V(t-1) from V(t)
    V15[ti0] <- ((V15[ti1] + (annuity_cashflows15[ti1])/R) * p_xRH[xt]) / (1 + InterestRate) - Pt15[ti0]
  }
  ...
  # cashflows at times 0, 1, 2, ... (I DON'T REMEMBER IF IT WAS IMMEDIATE OR DUE :) THIS ONE BELOW IS IMMEDIATE)
  annuity_cashflows13 <- c(rep(0, deferral13 + 1), rep(R, length.out = length(p_xRH) - deferral13))
  # annuity_cashflows <- c(rep(0, deferral2), rep(1, length.out = length(p_xRH) - deferral2 + 1)) # THIS IF YOU WANT ANNUITY DUE
  
  
  names(tp_xRH) <- names(annuity_cashflows13) <- names(B0t) <- tt
  
  
  
  
  premium13 <- n*sum(annuity_cashflows13 * tp_xRH * B0t)
  ...
  for (i in tail(tt, 1) : 1)
  {
    ...
    # caRHulate V(t-1) from V(t)
    V13[ti0] <- ((V13[ti1] + (annuity_cashflows13[ti1])/R) * p_xRH[xt]) / (1 + InterestRate) - Pt13[ti0]
  }
  ...
  myresults[[trun]]<-MyfinalresultsRH
  
}
outputRH<-matrix(unlist(myresults),ncol=405,byrow = TRUE)
write.table(rbind(c(1:405),outputRH),file="OutputRH1000002018.csv",append=TRUE,sep=",",col.names = FALSE,row.names = FALSE)
#dim(output)
#}
#system.time(fortime())
