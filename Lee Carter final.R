...
#for (trun in 1:totalruns) {
  ...
  #Here we first fit LC model to EW date. The code is from StMoMo vignette
  ...
  LCfor <- forecast(LCfit, h = 37)
  
  #Here I create the column of mortality rates (q_x) from the central forecasts for the cohort that is 65 in 2012. This is used to caLCulate premium and reserves
  q_xLC <- extractCohort(A = LCfor$rates, age = 65, period = 2012)
  
  ######## Here I linearly interpolated the  mortality rates from age 100 to age 120 (when q_x=1)
  
  #coefficient<-(1-q_xLC1[36,1])/20
  
  # quadratic interpolation
  
  
  quadrinterp <- poly.calc(x = c(99, 100, 120), y = c(q_xLC[35], q_xLC[36], 1))
  q_xLC[36 + (1 : 20)] <- predict(quadrinterp, 101 : 120)
  #points(q_xLC, col = "red")
  
  
  #q_xLC3=c()
  #for (i in 2:20) {
  # q_xLC3[1]<-q_xLC1[36,1]+coefficient
  # q_xLC3[i]<-q_xLC3[(i-1)]+coefficient
  
  #}
  
  #q_xLC2<-as.data.frame(q_xLC3)
  #colnames(q_xLC2)<-"CentralLC"
  #q_xLC<-rbind(q_xLC1,q_xLC2)
  
  ######## 
  
  #Сутекфд urvival rates
  p_xLC<-1-q_xLC
  
  
  # Names is differen here. "Central LC"
  names(q_xLC) <- names(p_xLC) <- seq(65, 120)
  
  
  
  tt <- seq(length(p_xLC))
  
  # cumulative survival rates (USE q_xLC AS A VECTOR RATHER THAN A DATA FRAME)
  tp_xLC <- cumprod(p_xLC)
  ...
  #Here I simulate  future mortality rates
  
  LCsim <- simulate(LCfit, nsim = runs, h = 37, gc.order = c(1, 1, 0))
  ...
  LCsim2012<- extractCohort(LCsim$rates, age = EntryAge, period = 2012)
  forinterpolation<-as.matrix(rbind(LCsim2012[35,], LCsim2012[36,], rep(1,runs)))
  ...
  LCcohort2012<-rbind(LCsim2012, forsimulations)
  LCcohort2012[LCcohort2012 > 1] <-1 
  rownames(LCcohort2012)<-65:120
  
  ######## Now take average of rows in LCcohort2012/survival rates
  LCmeans<-1-rowMeans(LCcohort2012, na.rm = FALSE, dims = 1)
  
  probs<-c(LCmeans[1],LCmeans[2]*tp_xLC[1],LCmeans[3]*tp_xLC[2],LCmeans[4]*tp_xLC[3],LCmeans[5]*tp_xLC[4],LCmeans[6]*tp_xLC[5],
           LCmeans[7]*tp_xLC[6],LCmeans[8]*tp_xLC[7],LCmeans[9]*tp_xLC[8],LCmeans[10]*tp_xLC[9],LCmeans[11]*tp_xLC[10],LCmeans[12]*tp_xLC[11],
           LCmeans[13]*tp_xLC[12],LCmeans[14]*tp_xLC[13],LCmeans[15]*tp_xLC[14],LCmeans[16]*tp_xLC[15],LCmeans[17]*tp_xLC[16],
           LCmeans[18]*tp_xLC[17],LCmeans[19]*tp_xLC[18],LCmeans[20]*tp_xLC[19],LCmeans[21]*tp_xLC[20],LCmeans[22]*tp_xLC[21],
           LCmeans[23]*tp_xLC[22],LCmeans[24]*tp_xLC[23],LCmeans[25]*tp_xLC[24],LCmeans[26]*tp_xLC[25],LCmeans[27]*tp_xLC[26],
           LCmeans[28]*tp_xLC[27],LCmeans[29]*tp_xLC[28],LCmeans[30]*tp_xLC[29],LCmeans[31]*tp_xLC[30],LCmeans[32]*tp_xLC[31],
           LCmeans[33]*tp_xLC[32],LCmeans[34]*tp_xLC[33],LCmeans[35]*tp_xLC[34],LCmeans[36]*tp_xLC[35],LCmeans[37]*tp_xLC[36],
           LCmeans[38]*tp_xLC[37],LCmeans[39]*tp_xLC[38],LCmeans[40]*tp_xLC[39],LCmeans[41]*tp_xLC[40],LCmeans[42]*tp_xLC[41],
           LCmeans[43]*tp_xLC[42],LCmeans[44]*tp_xLC[43],LCmeans[45]*tp_xLC[44],LCmeans[46]*tp_xLC[45],LCmeans[47]*tp_xLC[46],
           LCmeans[48]*tp_xLC[47],LCmeans[49]*tp_xLC[48],LCmeans[50]*tp_xLC[49],LCmeans[51]*tp_xLC[50],LCmeans[52]*tp_xLC[51],
           LCmeans[53]*tp_xLC[52],LCmeans[54]*tp_xLC[53],LCmeans[55]*tp_xLC[54],LCmeans[56]*tp_xLC[55])
  
  # cashflows at times 0, 1, 2, ... (I DON'T REMEMBER IF IT WAS IMMEDIATE OR DUE :) THIS ONE BELOW IS IMMEDIATE)
  annuity_cashflows <- c(rep(R, length.out = length(p_xLC) - deferral))
  # annuity_cashflows <- c(rep(0, deferral), rep(1, length.out = length(p_xLC) - deferral + 1)) # THIS IF YOU WANT ANNUITY DUE
  
  
  # discount factors
  B0t <- (1 + InterestRate) ^ - tt
  
  names(tp_xLC) <- names(annuity_cashflows) <- names(B0t) <- tt
  #Here add last probability expectation
  a_x <- n*sum(annuity_cashflows * tp_xLC * B0t)
  ...
  survivals[1,]<-n
  for (i in 1:56) {
    survivals[(i+1),]<-rbinom(runs,survivals[i,],1-LCcohort2012[i,])
  ...
  lr1st<-rep(0,runs)
  #CaLCulate random payments to policyholders considering random number of alives
  ...
  #CaLCulate random portfolio fund at times t, i.e. Z_t
  ...
  #CaLCulate solvency margin that needs to be added in the beginning in order to ensure solvence at time t. This is basically a quantile of dicounted deficit
  ...
  # cashflows at times 0, 1, 2, ... (I DON'T REMEMBER IF IT WAS IMMEDIATE OR DUE :) THIS ONE BELOW IS IMMEDIATE)
  annuity_cashflows2 <- c(rep(0, deferral2 + 1), rep(R, length.out = length(p_xLC) - deferral2))
  # annuity_cashflows <- c(rep(0, deferral2), rep(1, length.out = length(p_xLC) - deferral2 + 1)) # THIS IF YOU WANT ANNUITY DUE
  
  
  names(tp_xLC) <- names(annuity_cashflows2) <- names(B0t) <- tt
  
  
  
  
  premium2 <- n*sum(annuity_cashflows2 * tp_xLC * B0t)
  ...
  for (i in tail(tt, 1) : 1)
  {
    ...
    # caLCulate V(t-1) from V(t)
    V2[ti0] <- ((V2[ti1] + (annuity_cashflows2[ti1])/R) * p_xLC[xt]) / (1 + InterestRate) - Pt2[ti0]
  }
  ...
  # cashflows at times 0, 1, 2, ... (I DON'T REMEMBER IF IT WAS IMMEDIATE OR DUE :) THIS ONE BELOW IS IMMEDIATE)
  annuity_cashflows3 <- c(rep(0, deferral3 + 1), rep(R, length.out = length(p_xLC) - deferral3))
  # annuity_cashflows <- c(rep(0, deferral2), rep(1, length.out = length(p_xLC) - deferral2 + 1)) # THIS IF YOU WANT ANNUITY DUE
  
  
  names(tp_xLC) <- names(annuity_cashflows3) <- names(B0t) <- tt
  
  
  
  
  premium3 <- n*sum(annuity_cashflows3 * tp_xLC * B0t)
  ...
  for (i in tail(tt, 1) : 1)
  {
    ...
    # caLCulate V(t-1) from V(t)
    V3[ti0] <- ((V3[ti1] + (annuity_cashflows3[ti1])/R) * p_xLC[xt]) / (1 + InterestRate) - Pt3[ti0]
  }
  ...
  # cashflows at times 0, 1, 2, ... (I DON'T REMEMBER IF IT WAS IMMEDIATE OR DUE :) THIS ONE BELOW IS IMMEDIATE)
  annuity_cashflows4 <- c(rep(0, deferral4 + 1), rep(R, length.out = length(p_xLC) - deferral4))
  # annuity_cashflows <- c(rep(0, deferral2), rep(1, length.out = length(p_xLC) - deferral2 + 1)) # THIS IF YOU WANT ANNUITY DUE
  
  
  names(tp_xLC) <- names(annuity_cashflows4) <- names(B0t) <- tt
  
  
  
  
  premium4 <- n*sum(annuity_cashflows4 * tp_xLC * B0t)
  ...
  for (i in tail(tt, 1) : 1)
  {
    ...
    # caLCulate V(t-1) from V(t)
    V4[ti0] <- ((V4[ti1] + (annuity_cashflows4[ti1])/R) * p_xLC[xt]) / (1 + InterestRate) - Pt4[ti0]
  }
  ...
  # cashflows at times 0, 1, 2, ... (I DON'T REMEMBER IF IT WAS IMMEDIATE OR DUE :) THIS ONE BELOW IS IMMEDIATE)
  annuity_cashflows5 <- c(rep(0, deferral5 + 1), rep(R, length.out = length(p_xLC) - deferral5))
  # annuity_cashflows <- c(rep(0, deferral2), rep(1, length.out = length(p_xLC) - deferral2 + 1)) # THIS IF YOU WANT ANNUITY DUE
  
  
  names(tp_xLC) <- names(annuity_cashflows5) <- names(B0t) <- tt
  
  
  
  
  premium5 <- n*sum(annuity_cashflows5 * tp_xLC * B0t)
  ...
  for (i in tail(tt, 1) : 1)
  {
    ...
    # caLCulate V(t-1) from V(t)
    V5[ti0] <- ((V5[ti1] + (annuity_cashflows5[ti1])/R) * p_xLC[xt]) / (1 + InterestRate) - Pt5[ti0]
  }
  ...
  # cashflows at times 0, 1, 2, ... (I DON'T REMEMBER IF IT WAS IMMEDIATE OR DUE :) THIS ONE BELOW IS IMMEDIATE)
  annuity_cashflows6 <- c(rep(0, deferral6 + 1), rep(R, length.out = length(p_xLC) - deferral6))
  # annuity_cashflows <- c(rep(0, deferral2), rep(1, length.out = length(p_xLC) - deferral2 + 1)) # THIS IF YOU WANT ANNUITY DUE
  
  
  names(tp_xLC) <- names(annuity_cashflows6) <- names(B0t) <- tt
  
  
  
  
  premium6 <- n*sum(annuity_cashflows6 * tp_xLC * B0t)
  ...
  for (i in tail(tt, 1) : 1)
  {
    ...
    # caLCulate V(t-1) from V(t)
    V6[ti0] <- ((V6[ti1] + (annuity_cashflows6[ti1])/R) * p_xLC[xt]) / (1 + InterestRate) - Pt6[ti0]
  }
  ...
  # cashflows at times 0, 1, 2, ... (I DON'T REMEMBER IF IT WAS IMMEDIATE OR DUE :) THIS ONE BELOW IS IMMEDIATE)
  annuity_cashflows7 <- c(rep(0, deferral7 + 1), rep(R, length.out = length(p_xLC) - deferral7))
  # annuity_cashflows <- c(rep(0, deferral2), rep(1, length.out = length(p_xLC) - deferral2 + 1)) # THIS IF YOU WANT ANNUITY DUE
  
  
  names(tp_xLC) <- names(annuity_cashflows7) <- names(B0t) <- tt
  
  
  
  
  premium7 <- n*sum(annuity_cashflows7 * tp_xLC * B0t)
  ...
  for (i in tail(tt, 1) : 1)
  {
    ...
    # caLCulate V(t-1) from V(t)
    V7[ti0] <- ((V7[ti1] + (annuity_cashflows7[ti1])/R) * p_xLC[xt]) / (1 + InterestRate) - Pt7[ti0]
  }
  ...
  # cashflows at times 0, 1, 2, ... (I DON'T REMEMBER IF IT WAS IMMEDIATE OR DUE :) THIS ONE BELOW IS IMMEDIATE)
  annuity_cashflows8 <- c(rep(0, deferral8 + 1), rep(R, length.out = length(p_xLC) - deferral8))
  # annuity_cashflows <- c(rep(0, deferral2), rep(1, length.out = length(p_xLC) - deferral2 + 1)) # THIS IF YOU WANT ANNUITY DUE
  
  
  names(tp_xLC) <- names(annuity_cashflows8) <- names(B0t) <- tt
  
  
  
  
  premium8 <- n*sum(annuity_cashflows8 * tp_xLC * B0t)
  ...
  for (i in tail(tt, 1) : 1)
  {
    ...
    # caLCulate V(t-1) from V(t)
    V8[ti0] <- ((V8[ti1] + (annuity_cashflows8[ti1])/R) * p_xLC[xt]) / (1 + InterestRate) - Pt8[ti0]
  }
  ...
  # cashflows at times 0, 1, 2, ... (I DON'T REMEMBER IF IT WAS IMMEDIATE OR DUE :) THIS ONE BELOW IS IMMEDIATE)
  annuity_cashflows9 <- c(rep(0, deferral9 + 1), rep(R, length.out = length(p_xLC) - deferral9))
  # annuity_cashflows <- c(rep(0, deferral2), rep(1, length.out = length(p_xLC) - deferral2 + 1)) # THIS IF YOU WANT ANNUITY DUE
  
  
  names(tp_xLC) <- names(annuity_cashflows9) <- names(B0t) <- tt
  
  
  
  
  premium9 <- n*sum(annuity_cashflows9 * tp_xLC * B0t)
  ...
  for (i in tail(tt, 1) : 1)
  {
    ...
    # caLCulate V(t-1) from V(t)
    V9[ti0] <- ((V9[ti1] + (annuity_cashflows9[ti1])/R) * p_xLC[xt]) / (1 + InterestRate) - Pt9[ti0]
  }
  ...
  # cashflows at times 0, 1, 2, ... (I DON'T REMEMBER IF IT WAS IMMEDIATE OR DUE :) THIS ONE BELOW IS IMMEDIATE)
  annuity_cashflows10 <- c(rep(0, deferral10 + 1), rep(R, length.out = length(p_xLC) - deferral10))
  # annuity_cashflows <- c(rep(0, deferral2), rep(1, length.out = length(p_xLC) - deferral2 + 1)) # THIS IF YOU WANT ANNUITY DUE
  
  
  names(tp_xLC) <- names(annuity_cashflows10) <- names(B0t) <- tt
  
  
  
  
  premium10 <- n*sum(annuity_cashflows10 * tp_xLC * B0t)
  ...
  for (i in tail(tt, 1) : 1)
  {
    ...
    # caLCulate V(t-1) from V(t)
    V10[ti0] <- ((V10[ti1] + (annuity_cashflows10[ti1])/R) * p_xLC[xt]) / (1 + InterestRate) - Pt10[ti0]
  }
  ...
  # cashflows at times 0, 1, 2, ... (I DON'T REMEMBER IF IT WAS IMMEDIATE OR DUE :) THIS ONE BELOW IS IMMEDIATE)
  annuity_cashflows11 <- c(rep(0, deferral11 + 1), rep(R, length.out = length(p_xLC) - deferral11))
  # annuity_cashflows <- c(rep(0, deferral2), rep(1, length.out = length(p_xLC) - deferral2 + 1)) # THIS IF YOU WANT ANNUITY DUE
  
  
  names(tp_xLC) <- names(annuity_cashflows11) <- names(B0t) <- tt
  
  
  
  
  premium11 <- n*sum(annuity_cashflows11 * tp_xLC * B0t)
  ...
  for (i in tail(tt, 1) : 1)
  {
    ...
    # caLCulate V(t-1) from V(t)
    V11[ti0] <- ((V11[ti1] + (annuity_cashflows11[ti1])/R) * p_xLC[xt]) / (1 + InterestRate) - Pt11[ti0]
  }
  ...
  # cashflows at times 0, 1, 2, ... (I DON'T REMEMBER IF IT WAS IMMEDIATE OR DUE :) THIS ONE BELOW IS IMMEDIATE)
  annuity_cashflows14 <- c(rep(0, deferral14 + 1), rep(R, length.out = length(p_xLC) - deferral14))
  # annuity_cashflows <- c(rep(0, deferral2), rep(1, length.out = length(p_xLC) - deferral2 + 1)) # THIS IF YOU WANT ANNUITY DUE
  
  
  names(tp_xLC) <- names(annuity_cashflows14) <- names(B0t) <- tt
  
  
  
  
  premium14 <- n*sum(annuity_cashflows14 * tp_xLC * B0t)
  ...
  for (i in tail(tt, 1) : 1)
  {
    ...
    # caLCulate V(t-1) from V(t)
    V14[ti0] <- ((V14[ti1] + (annuity_cashflows14[ti1])/R) * p_xLC[xt]) / (1 + InterestRate) - Pt14[ti0]
  }
  ...
  # cashflows at times 0, 1, 2, ... (I DON'T REMEMBER IF IT WAS IMMEDIATE OR DUE :) THIS ONE BELOW IS IMMEDIATE)
  annuity_cashflows12 <- c(rep(0, deferral12 + 1), rep(R, length.out = length(p_xLC) - deferral12))
  # annuity_cashflows <- c(rep(0, deferral2), rep(1, length.out = length(p_xLC) - deferral2 + 1)) # THIS IF YOU WANT ANNUITY DUE
  
  
  names(tp_xLC) <- names(annuity_cashflows12) <- names(B0t) <- tt
  
  
  
  
  premium12 <- n*sum(annuity_cashflows12 * tp_xLC * B0t)
  ...
  for (i in tail(tt, 1) : 1)
  {
    ...
    # caLCulate V(t-1) from V(t)
    V12[ti0] <- ((V12[ti1] + (annuity_cashflows12[ti1])/R) * p_xLC[xt]) / (1 + InterestRate) - Pt12[ti0]
  }
  ...
  # cashflows at times 0, 1, 2, ... (I DON'T REMEMBER IF IT WAS IMMEDIATE OR DUE :) THIS ONE BELOW IS IMMEDIATE)
  annuity_cashflows15 <- c(rep(0, deferral15 + 1), rep(R, length.out = length(p_xLC) - deferral15))
  # annuity_cashflows <- c(rep(0, deferral2), rep(1, length.out = length(p_xLC) - deferral2 + 1)) # THIS IF YOU WANT ANNUITY DUE
  
  
  names(tp_xLC) <- names(annuity_cashflows15) <- names(B0t) <- tt
  
  
  
  
  premium15 <- n*sum(annuity_cashflows15 * tp_xLC * B0t)
  ...
  for (i in tail(tt, 1) : 1)
  {
    ...
    # caLCulate V(t-1) from V(t)
    V15[ti0] <- ((V15[ti1] + (annuity_cashflows15[ti1])/R) * p_xLC[xt]) / (1 + InterestRate) - Pt15[ti0]
  }
  ...
  # cashflows at times 0, 1, 2, ... (I DON'T REMEMBER IF IT WAS IMMEDIATE OR DUE :) THIS ONE BELOW IS IMMEDIATE)
  annuity_cashflows13 <- c(rep(0, deferral13 + 1), rep(R, length.out = length(p_xLC) - deferral13))
  # annuity_cashflows <- c(rep(0, deferral2), rep(1, length.out = length(p_xLC) - deferral2 + 1)) # THIS IF YOU WANT ANNUITY DUE
  
  
  names(tp_xLC) <- names(annuity_cashflows13) <- names(B0t) <- tt
  
  
  
  
  premium13 <- n*sum(annuity_cashflows13 * tp_xLC * B0t)
  ...
  for (i in tail(tt, 1) : 1)
  {
    ...
    # caLCulate V(t-1) from V(t)
    V13[ti0] <- ((V13[ti1] + (annuity_cashflows13[ti1])/R) * p_xLC[xt]) / (1 + InterestRate) - Pt13[ti0]
  }
  ...
  myresults[[trun]]<-MyfinalresultsLC
  
}
outputLC<-matrix(unlist(myresults),ncol=405,byrow = TRUE)
write.table(rbind(c(1:405),outputLC),file="OutputLC1002018.csv",append=TRUE,sep=",",col.names = FALSE,row.names = FALSE)
#dim(output)
#}
#system.time(fortime())
