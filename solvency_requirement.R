...
# annuity age
annuity_age <- entry_age + deferral

ages <- entry_age:120

# here we first fit APC model to EW date. The code is from StMoMo vignette

EWMaleIniData <- central2initial(EWMaleData)

ages_fit <- entry_age:100

wxt <- genWeightMat(ages = ages_fit, years = EWMaleIniData$years, clip = 3)
...
APC_for <- forecast(APCfit,
                    h = 37,
                    gc.order = c(1, 1, 0))


#Here I create the column of mortality rates (q_x) from the central forecasts for the cohort that is 65 in 2012. This is used to caAPCulate premium and reserves
q_xAPC <- extractCohort(A = APCfor$rates,
                        age = 65,
                        period = 2012)

######## Here I linearly interpolated the  mortality rates from age 100 to age 120 (when q_x=1)

#coefficient<-(1-q_xAPC1[36,1])/20

# quadratic interpolation


quadrinterp <- poly.calc(x = c(99, 100, 120), y = c(q_xAPC[35], q_xAPC[36], 1))
q_xAPC[36 + (1 : 20)] <- predict(quadrinterp, 101 : 120)
#points(q_xAPC, col = "red")


#q_xAPC3=c()
#for (i in 2:20) {
# q_xAPC3[1]<-q_xAPC1[36,1]+coefficient
# q_xAPC3[i]<-q_xAPC3[(i-1)]+coefficient

#}

#q_xAPC2<-as.data.frame(q_xAPC3)
#colnames(q_xAPC2)<-"CentralAPC"
#q_xAPC<-rbind(q_xAPC1,q_xAPC2)
...
# names is different here. "Central APC"
names(q_xAPC) <- names(p_xAPC) <- seq(65, 120)


tt <- seq(0, length(p_xAPC))

# cumulative survival rates (USE q_xAPC AS A VECTOR RATHER THAN A DATA FRAME)
tp_xAPC <- cumprod(c(1, p_xAPC))

# cashflows at times 0, 1, 2, ... (I DON'T REMEMBER IF IT WAS IMMEDIATE OR DUE :) THIS ONE BELOW IS IMMEDIATE)
annuity_cashflows <- c(rep(0, deferral + 1), rep(R, length.out = length(p_xAPC) - deferral))
# annuity_cashflows <- c(rep(0, deferral), rep(1, length.out = length(p_xAPC) - deferral + 1)) # THIS IF YOU WANT ANNUITY DUE

# discount factors
B0t <- (1 + InterestRate) ^ - tt

names(tp_xAPC) <- names(annuity_cashflows) <- names(B0t) <- tt

premium <- n*sum(annuity_cashflows * tp_xAPC * B0t)




for (trun in 1:total_runs){
  
  
  
  ############################
  # CaAPCulation backward #####
  ############################
  ...
  for (i in tail(tt, 1) : 1)
  {
    ...
    # caAPCulate V(t-1) from V(t)
    V[ti0] <- ((V[ti1] + (annuity_cashflows[ti1])/R) * p_xAPC[xt]) / (1 + InterestRate) - Pt[ti0]
  }
  ...
  APCsim <- simulate(APCfit, nsim = runs, h = 37, gc.order = c(1, 1, 0))
  ...
  APCsim2012<- extractCohort(APCsim$rates, age = EntryAge, period = 2012)
  forinterpolation<-as.matrix(rbind(APCsim2012[35,], APCsim2012[36,], rep(1,runs)))
  ...
  APCcohort2012<-rbind(APCsim2012, forsimulations)
  APCcohort2012[APCcohort2012 > 1] <-1 
  rownames(APCcohort2012)<-65:120
  ...
  survivals[1,]<-n
  for (i in 1:56) {
    survivals[(i+1),]<-rbinom(runs,survivals[i,],1-APCcohort2012[i,])
  ...
  lr1st<-rep(0,runs)
  #CaAPCulate random payments to policyholders considering random number of alives
  ...
  #CaAPCulate random portfolio fund at times t, i.e. Z_t
  ...
  #CaAPCulate solvency margin that needs to be added in the beginning in order to ensure solvence at time t. This is basically a quantile of dicounted deficit
  ...
  # cashflows at times 0, 1, 2, ... (I DON'T REMEMBER IF IT WAS IMMEDIATE OR DUE :) THIS ONE BELOW IS IMMEDIATE)
  annuity_cashflows2 <- c(rep(0, deferral2 + 1), rep(R, length.out = length(p_xAPC) - deferral2))
  # annuity_cashflows <- c(rep(0, deferral2), rep(1, length.out = length(p_xAPC) - deferral2 + 1)) # THIS IF YOU WANT ANNUITY DUE
  
  
  names(tp_xAPC) <- names(annuity_cashflows2) <- names(B0t) <- tt
  
  
  
  
  premium2 <- n*sum(annuity_cashflows2 * tp_xAPC * B0t)
  ...
  for (i in tail(tt, 1) : 1)
  {
    ...
    # caAPCulate V(t-1) from V(t)
    V2[ti0] <- ((V2[ti1] + (annuity_cashflows2[ti1])/R) * p_xAPC[xt]) / (1 + InterestRate) - Pt2[ti0]
  }
  ...
  # cashflows at times 0, 1, 2, ... (I DON'T REMEMBER IF IT WAS IMMEDIATE OR DUE :) THIS ONE BELOW IS IMMEDIATE)
  annuity_cashflows3 <- c(rep(0, deferral3 + 1), rep(R, length.out = length(p_xAPC) - deferral3))
  # annuity_cashflows <- c(rep(0, deferral2), rep(1, length.out = length(p_xAPC) - deferral2 + 1)) # THIS IF YOU WANT ANNUITY DUE
  
  
  names(tp_xAPC) <- names(annuity_cashflows3) <- names(B0t) <- tt
  
  
  
  
  premium3 <- n*sum(annuity_cashflows3 * tp_xAPC * B0t)
  ...
  for (i in tail(tt, 1) : 1)
  {
    ...
    # caAPCulate V(t-1) from V(t)
    V3[ti0] <- ((V3[ti1] + (annuity_cashflows3[ti1])/R) * p_xAPC[xt]) / (1 + InterestRate) - Pt3[ti0]
  }
  ...
  # cashflows at times 0, 1, 2, ... (I DON'T REMEMBER IF IT WAS IMMEDIATE OR DUE :) THIS ONE BELOW IS IMMEDIATE)
  annuity_cashflows4 <- c(rep(0, deferral4 + 1), rep(R, length.out = length(p_xAPC) - deferral4))
  # annuity_cashflows <- c(rep(0, deferral2), rep(1, length.out = length(p_xAPC) - deferral2 + 1)) # THIS IF YOU WANT ANNUITY DUE
  
  
  names(tp_xAPC) <- names(annuity_cashflows4) <- names(B0t) <- tt
  
  
  
  
  premium4 <- n*sum(annuity_cashflows4 * tp_xAPC * B0t)
  ...
  for (i in tail(tt, 1) : 1)
  {
    ...
    # caAPCulate V(t-1) from V(t)
    V4[ti0] <- ((V4[ti1] + (annuity_cashflows4[ti1])/R) * p_xAPC[xt]) / (1 + InterestRate) - Pt4[ti0]
  }
  ...
  # cashflows at times 0, 1, 2, ... (I DON'T REMEMBER IF IT WAS IMMEDIATE OR DUE :) THIS ONE BELOW IS IMMEDIATE)
  annuity_cashflows5 <- c(rep(0, deferral5 + 1), rep(R, length.out = length(p_xAPC) - deferral5))
  # annuity_cashflows <- c(rep(0, deferral2), rep(1, length.out = length(p_xAPC) - deferral2 + 1)) # THIS IF YOU WANT ANNUITY DUE
  
  
  names(tp_xAPC) <- names(annuity_cashflows5) <- names(B0t) <- tt
  
  
  
  
  premium5 <- n*sum(annuity_cashflows5 * tp_xAPC * B0t)
  ...
  for (i in tail(tt, 1) : 1)
  {
    ...
    # caAPCulate V(t-1) from V(t)
    V5[ti0] <- ((V5[ti1] + (annuity_cashflows5[ti1])/R) * p_xAPC[xt]) / (1 + InterestRate) - Pt5[ti0]
  }
  ...
  # cashflows at times 0, 1, 2, ... (I DON'T REMEMBER IF IT WAS IMMEDIATE OR DUE :) THIS ONE BELOW IS IMMEDIATE)
  annuity_cashflows6 <- c(rep(0, deferral6 + 1), rep(R, length.out = length(p_xAPC) - deferral6))
  # annuity_cashflows <- c(rep(0, deferral2), rep(1, length.out = length(p_xAPC) - deferral2 + 1)) # THIS IF YOU WANT ANNUITY DUE
  
  
  names(tp_xAPC) <- names(annuity_cashflows6) <- names(B0t) <- tt
  
  
  
  
  premium6 <- n*sum(annuity_cashflows6 * tp_xAPC * B0t)
  ...
  for (i in tail(tt, 1) : 1)
  {
    ...
    # caAPCulate V(t-1) from V(t)
    V6[ti0] <- ((V6[ti1] + (annuity_cashflows6[ti1])/R) * p_xAPC[xt]) / (1 + InterestRate) - Pt6[ti0]
  }
  ...
  # cashflows at times 0, 1, 2, ... (I DON'T REMEMBER IF IT WAS IMMEDIATE OR DUE :) THIS ONE BELOW IS IMMEDIATE)
  annuity_cashflows7 <- c(rep(0, deferral7 + 1), rep(R, length.out = length(p_xAPC) - deferral7))
  # annuity_cashflows <- c(rep(0, deferral2), rep(1, length.out = length(p_xAPC) - deferral2 + 1)) # THIS IF YOU WANT ANNUITY DUE
  
  
  names(tp_xAPC) <- names(annuity_cashflows7) <- names(B0t) <- tt
  
  
  
  
  premium7 <- n*sum(annuity_cashflows7 * tp_xAPC * B0t)
  ...
  for (i in tail(tt, 1) : 1)
  {
    ...
    # caAPCulate V(t-1) from V(t)
    V7[ti0] <- ((V7[ti1] + (annuity_cashflows7[ti1])/R) * p_xAPC[xt]) / (1 + InterestRate) - Pt7[ti0]
  }
  ...
  # cashflows at times 0, 1, 2, ... (I DON'T REMEMBER IF IT WAS IMMEDIATE OR DUE :) THIS ONE BELOW IS IMMEDIATE)
  annuity_cashflows8 <- c(rep(0, deferral8 + 1), rep(R, length.out = length(p_xAPC) - deferral8))
  # annuity_cashflows <- c(rep(0, deferral2), rep(1, length.out = length(p_xAPC) - deferral2 + 1)) # THIS IF YOU WANT ANNUITY DUE
  
  
  names(tp_xAPC) <- names(annuity_cashflows8) <- names(B0t) <- tt
  
  
  
  
  premium8 <- n*sum(annuity_cashflows8 * tp_xAPC * B0t)
  ...
  for (i in tail(tt, 1) : 1)
  {
    ...
    # caAPCulate V(t-1) from V(t)
    V8[ti0] <- ((V8[ti1] + (annuity_cashflows8[ti1])/R) * p_xAPC[xt]) / (1 + InterestRate) - Pt8[ti0]
  }
  ...
  # cashflows at times 0, 1, 2, ... (I DON'T REMEMBER IF IT WAS IMMEDIATE OR DUE :) THIS ONE BELOW IS IMMEDIATE)
  annuity_cashflows9 <- c(rep(0, deferral9 + 1), rep(R, length.out = length(p_xAPC) - deferral9))
  # annuity_cashflows <- c(rep(0, deferral2), rep(1, length.out = length(p_xAPC) - deferral2 + 1)) # THIS IF YOU WANT ANNUITY DUE
  
  
  names(tp_xAPC) <- names(annuity_cashflows9) <- names(B0t) <- tt
  
  
  
  
  premium9 <- n*sum(annuity_cashflows9 * tp_xAPC * B0t)
  ...
  for (i in tail(tt, 1) : 1)
  {
    ...
    # caAPCulate V(t-1) from V(t)
    V9[ti0] <- ((V9[ti1] + (annuity_cashflows9[ti1])/R) * p_xAPC[xt]) / (1 + InterestRate) - Pt9[ti0]
  }
  ...
  # cashflows at times 0, 1, 2, ... (I DON'T REMEMBER IF IT WAS IMMEDIATE OR DUE :) THIS ONE BELOW IS IMMEDIATE)
  annuity_cashflows10 <- c(rep(0, deferral10 + 1), rep(R, length.out = length(p_xAPC) - deferral10))
  # annuity_cashflows <- c(rep(0, deferral2), rep(1, length.out = length(p_xAPC) - deferral2 + 1)) # THIS IF YOU WANT ANNUITY DUE
  
  
  names(tp_xAPC) <- names(annuity_cashflows10) <- names(B0t) <- tt
  
  
  
  
  premium10 <- n*sum(annuity_cashflows10 * tp_xAPC * B0t)
  ...
  for (i in tail(tt, 1) : 1)
  {
    ...
    # caAPCulate V(t-1) from V(t)
    V10[ti0] <- ((V10[ti1] + (annuity_cashflows10[ti1])/R) * p_xAPC[xt]) / (1 + InterestRate) - Pt10[ti0]
  }
  ...
  # cashflows at times 0, 1, 2, ... (I DON'T REMEMBER IF IT WAS IMMEDIATE OR DUE :) THIS ONE BELOW IS IMMEDIATE)
  annuity_cashflows11 <- c(rep(0, deferral11 + 1), rep(R, length.out = length(p_xAPC) - deferral11))
  # annuity_cashflows <- c(rep(0, deferral2), rep(1, length.out = length(p_xAPC) - deferral2 + 1)) # THIS IF YOU WANT ANNUITY DUE
  
  
  names(tp_xAPC) <- names(annuity_cashflows11) <- names(B0t) <- tt
  
  
  
  
  premium11 <- n*sum(annuity_cashflows11 * tp_xAPC * B0t)
  ...
  for (i in tail(tt, 1) : 1)
  {
    ...
    # caAPCulate V(t-1) from V(t)
    V11[ti0] <- ((V11[ti1] + (annuity_cashflows11[ti1])/R) * p_xAPC[xt]) / (1 + InterestRate) - Pt11[ti0]
  }
  ...
  # cashflows at times 0, 1, 2, ... (I DON'T REMEMBER IF IT WAS IMMEDIATE OR DUE :) THIS ONE BELOW IS IMMEDIATE)
  annuity_cashflows14 <- c(rep(0, deferral14 + 1), rep(R, length.out = length(p_xAPC) - deferral14))
  # annuity_cashflows <- c(rep(0, deferral2), rep(1, length.out = length(p_xAPC) - deferral2 + 1)) # THIS IF YOU WANT ANNUITY DUE
  
  
  names(tp_xAPC) <- names(annuity_cashflows14) <- names(B0t) <- tt
  
  
  
  
  premium14 <- n*sum(annuity_cashflows14 * tp_xAPC * B0t)
  ...
  for (i in tail(tt, 1) : 1)
  {
    ...
    # caAPCulate V(t-1) from V(t)
    V14[ti0] <- ((V14[ti1] + (annuity_cashflows14[ti1])/R) * p_xAPC[xt]) / (1 + InterestRate) - Pt14[ti0]
  }
  ...
  # cashflows at times 0, 1, 2, ... (I DON'T REMEMBER IF IT WAS IMMEDIATE OR DUE :) THIS ONE BELOW IS IMMEDIATE)
  annuity_cashflows12 <- c(rep(0, deferral12 + 1), rep(R, length.out = length(p_xAPC) - deferral12))
  # annuity_cashflows <- c(rep(0, deferral2), rep(1, length.out = length(p_xAPC) - deferral2 + 1)) # THIS IF YOU WANT ANNUITY DUE
  
  
  names(tp_xAPC) <- names(annuity_cashflows12) <- names(B0t) <- tt
  
  
  
  
  premium12 <- n*sum(annuity_cashflows12 * tp_xAPC * B0t)
  ...
  for (i in tail(tt, 1) : 1)
  {
    ...
    # caAPCulate V(t-1) from V(t)
    V12[ti0] <- ((V12[ti1] + (annuity_cashflows12[ti1])/R) * p_xAPC[xt]) / (1 + InterestRate) - Pt12[ti0]
  }
  ...
  # cashflows at times 0, 1, 2, ... (I DON'T REMEMBER IF IT WAS IMMEDIATE OR DUE :) THIS ONE BELOW IS IMMEDIATE)
  annuity_cashflows15 <- c(rep(0, deferral15 + 1), rep(R, length.out = length(p_xAPC) - deferral15))
  # annuity_cashflows <- c(rep(0, deferral2), rep(1, length.out = length(p_xAPC) - deferral2 + 1)) # THIS IF YOU WANT ANNUITY DUE
  
  
  names(tp_xAPC) <- names(annuity_cashflows15) <- names(B0t) <- tt
  
  
  
  
  premium15 <- n*sum(annuity_cashflows15 * tp_xAPC * B0t)
  ...
  for (i in tail(tt, 1) : 1)
  {
    ...
    # caAPCulate V(t-1) from V(t)
    V15[ti0] <- ((V15[ti1] + (annuity_cashflows15[ti1])/R) * p_xAPC[xt]) / (1 + InterestRate) - Pt15[ti0]
  }
  ...
  # cashflows at times 0, 1, 2, ... (I DON'T REMEMBER IF IT WAS IMMEDIATE OR DUE :) THIS ONE BELOW IS IMMEDIATE)
  annuity_cashflows13 <- c(rep(0, deferral13 + 1), rep(R, length.out = length(p_xAPC) - deferral13))
  # annuity_cashflows <- c(rep(0, deferral2), rep(1, length.out = length(p_xAPC) - deferral2 + 1)) # THIS IF YOU WANT ANNUITY DUE
  
  
  names(tp_xAPC) <- names(annuity_cashflows13) <- names(B0t) <- tt
  
  
  
  
  premium13 <- n*sum(annuity_cashflows13 * tp_xAPC * B0t)
  ...
  for (i in tail(tt, 1) : 1)
  {
    ...
    # caAPCulate V(t-1) from V(t)
    V13[ti0] <- ((V13[ti1] + (annuity_cashflows13[ti1])/R) * p_xAPC[xt]) / (1 + InterestRate) - Pt13[ti0]
  }
  ...
  myresults[[trun]]<-MyfinalresultsAPC
  
}
outputAPC<-matrix(unlist(myresults),ncol=405,byrow = TRUE)
write.table(rbind(c(1:405),outputAPC),file="OutputAPC1002018.csv",append=TRUE,sep=",",col.names = FALSE,row.names = FALSE)
#dim(output)
#}
#system.time(fortime())
